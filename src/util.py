...
def bounded_float(string, min=None, max=None, extra_predicate=lambda x: False, allow_none = False):
    if allow_none and string is None: return string
    value = float(string)
    if extra_predicate(value) or (min <= value <= max):
        return value
    else:
        raise argparse.ArgumentTypeError(
            f'Value not in range {min or "-∞"}-{max or "∞"}. Please either keep it in range or leave it out.')

def path_string(string, suffix="", must_exist: bool = True):
...
def path_arg(string, suffix="", must_exist: bool = True):
    path = Path(string).resolve()
    if (not must_exist or path.exists()) and path.suffix in suffix:  # in returns true if string ==
        return path
    else:
